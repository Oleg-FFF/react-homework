    ...
    onSubmit = (e) => {
        ...
        if (this.userNameRef.current.value.trim() === "") {
            this.setState(prevState => {
                return {currentError: {...prevState.currentError, userName: 'This field can not be empty'}}
            });
        }
        if (this.userLastNameRef.current.value.trim() === "") {
            this.setState(prevState => {
                return {currentError: {...prevState.currentError, userLastName: 'This field can not be empty'}}
            });
        } else {
    ...
    render() {
        ...
        return (
            <form className={CN} onSubmit={this.onSubmit}>
                ...
                {/*    <div>Error <img className="error-img" src={ErrorImg} alt="error"/>{'This field can not be empty'}</div>)}*/}
